...
const ScrollToTop = () => {
  ...
  return isScrolled ? (
    <button
      onClick={moveToTop}
      className="!fixed right-8 xs:right-7 bottom-24 z-40 opacity-1 transition-all duration-300"
    >
    ...
    <button
      onClick={moveToTop}
      className="!fixed right-8 xs:right-7 bottom-10 z-40 opacity-0 transition-all duration-300"
    >
      <ChevronDoubleUpIcon className="w-10 h-10 xs:w-12 xs:h-12 p-1 hover:shadow-md text-custom-black hover:bg-custom-black hover:text-custom-beige hover:rounded-full transition-all duration-300 ease-in-out" />
    </button>
  );
